...
int readPacket_Flv(std::ifstream& file, int chunk_size, FILE* file_video_out)
{
    int tagType = 0;
    uint32_t ts = 0;
    uint32_t bodylen = -1;
            ...
            if (file.read((char*)buffer4.data(), bodysubLen)) {
                ...
                if (firstPacket == true)
                {
                    if (buffer3[6] == 0x09 && ((buffer4[1] == 0x01 && buffer4[2] == 0x00 && buffer4[3] == 0x00 /*&& buffer4[4] == 0x00*/) /*|| (buffer4[1] == 0x00 && buffer4[2] == 0x00 && buffer4[3] == 0x00 /*&& buffer4[4] == 0x00*//*)*/))
                    {
                        ...
                        //((bodysubLen >> 8) >> 8) & 0xff;
                        uint8_t tmp1 = (((bodylen) >> 8) >> 8) & 0xff;
                        uint8_t tmp2 = ((bodylen) >> 8) & 0xff;
                        uint8_t tmp3 = (bodylen) & 0xff;
                        vecMerged.push_back(tmp1);
                        vecMerged.push_back(tmp2);
                        vecMerged.push_back(tmp3);

                        uint32_t timestampbase = last_videots & 0xffffff;
                        uint8_t timestampExt = last_videots >> 24 & 0xff;
                    ...
                    else if (buffer3[6] == 0x08 && buffer4[1] == 0x00)
                    {
                        ...
                        uint32_t timestampbase = last_audiots & 0xffffff;
                        uint8_t timestampExt = last_audiots >> 24 & 0xff;
                    ...
                    else if (buffer3[6] == 0x12)
                    {
                        vecMerged.push_back(type);
                        //((bodysubLen >> 8) >> 8) & 0xff;
                        uint8_t tmplen1 = (((bodylen) >> 8) >> 8) & 0xff;
                        uint8_t tmplen2 = ((bodylen) >> 8) & 0xff;
                        uint8_t tmplen3 = (bodylen) & 0xff;
                        vecMerged.push_back(tmplen1);
                        vecMerged.push_back(tmplen2);
                        vecMerged.push_back(tmplen3);
    ...
    if (vecMerged.size() > 0)
    {
		...
		do {
			//vecMerged.erase(vecMerged.begin(), std::next(vecMerged.begin(), 4));
			written = fwrite(vecMerged.data() + written_bytes, 1, bodysubLenTmp, file_video_out);
			if (written != bodysubLenTmp) {
				std::cout << "Error writing to file nalu_size:" << bodysubLenTmp << ", written:" << written << std::endl;
			}
			if (0 == written)
			{
				return -1;
			}
            bodysubLenTmp -= written;
			written_bytes += written;
		} while (bodysubLenTmp != 0);
        bodysubLenTmp = vecMerged.size();
        vecMerged.clear();

        int32_t preDataLen = bodysubLenTmp;

        uint8_t tmp1 = ((((preDataLen) >> 8) >> 8) >> 8) & 0xff;
        uint8_t tmp2 = (((preDataLen) >> 8) >> 8) & 0xff;
        uint8_t tmp3 = ((preDataLen) >> 8) & 0xff;
        uint8_t tmp4 = (preDataLen) & 0xff;
        vecMerged.push_back(tmp1);
        vecMerged.push_back(tmp2);
        vecMerged.push_back(tmp3);
        vecMerged.push_back(tmp4);
        fwrite(vecMerged.data(), 1, 4, file_video_out);
        ...
        //index_video++;
        //video_out_bin.close();
        std::cout << "0 video packet count: " << index_video << ", preDataLen:" << preDataLen << std::endl;
    }
    ...
    if (vecMergedAudio.size() > 0)
    {
        ...
        do {
            //vecMerged.erase(vecMerged.begin(), std::next(vecMerged.begin(), 4));
            written = fwrite(vecMergedAudio.data() + written_bytes, 1, nalu_size_tmp, file_video_out);
            if (written != nalu_size_tmp) {
                std::cout << "Error writing to file nalu_size:" << nalu_size_tmp << ", written:" << written << std::endl;
            }
            if (0 == written)
        ...
        nalu_size_tmp = vecMergedAudio.size();
        vecMergedAudio.clear();

        int32_t preDataLen = nalu_size_tmp;

        uint8_t tmp1 = ((((preDataLen) >> 8) >> 8) >> 8) & 0xff;
        uint8_t tmp2 = (((preDataLen) >> 8) >> 8) & 0xff;
        uint8_t tmp3 = ((preDataLen) >> 8) & 0xff;
        uint8_t tmp4 = (preDataLen) & 0xff;
        vecMergedAudio.push_back(tmp1);
        vecMergedAudio.push_back(tmp2);
        vecMergedAudio.push_back(tmp3);
        vecMergedAudio.push_back(tmp4);
        fwrite(vecMergedAudio.data(), 1, 4, file_video_out);
...
int main() {
    ...
    FILE* file_video_out = fopen("test_rtmp_hevc.flv", "wb");
    ...
    fwrite(sHeader, 1, sizeof(sHeader), file_video_out);
